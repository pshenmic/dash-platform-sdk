...
describe('KeyPair', () => {
  ...
  describe('mnemonic', () => {
    test('should be able to get seed from mnemonic', async () => {
      const seed = await sdk.keyPair.mnemonicToSeed(mnemonic)
    ...
    // test('should be able to derive testnet identity private key', async () => {
    //   const seed = await sdk.keyPair.mnemonicToSeed(mnemonic)
    //   const hdKey = sdk.keyPair.seedToHdKey(seed)
    //   const identityHdKey = sdk.keyPair.deriveIdentityPrivateKey(hdKey, 0, 1, 'mainnet')
    //   //console.log(bytesToHex(identityHdKey.privateKey as Uint8Array))
    //
    ...
    //
    //     const wallet = await sdk.keyPair.seedToWallet(seed, { versions: 'testnet' })
    //
    //     expect(wallet.privateKey).toEqual(new Uint8Array([105, 213, 35, 36, 214, 9, 104, 32, 148, 57, 157, 10, 134, 251, 79, 152, 252, 58, 194, 48, 145, 102, 7, 252, 191, 69, 164, 97, 195, 136, 182, 93]))
    //   })
    //
    //   test('should be able to get wallet from mnemonic', async () => {
    //     const wallet = await sdk.keyPair.mnemonicToWallet(mnemonic, '', true, { versions: 'testnet' })
    //
    //     expect(wallet.privateKey).toEqual(new Uint8Array([105, 213, 35, 36, 214, 9, 104, 32, 148, 57, 157, 10, 134, 251, 79, 152, 252, 58, 194, 48, 145, 102, 7, 252, 191, 69, 164, 97, 195, 136, 182, 93]))
    //   })
    //
    //   test('should be able to get wallet for testnet from mnemonic without options', async () => {
    //     const wallet = await sdk.keyPair.mnemonicToWallet(mnemonic)
    //
    //     expect(wallet.privateKey).toEqual(new Uint8Array([105, 213, 35, 36, 214, 9, 104, 32, 148, 57, 157, 10, 134, 251, 79, 152, 252, 58, 194, 48, 145, 102, 7, 252, 191, 69, 164, 97, 195, 136, 182, 93]))
    //   })
    // })
    ...
    //     const wallet = await sdk.keyPair.mnemonicToWallet(mnemonic, '', true, { versions: 'testnet' })
    //
    //     const key = await sdk.keyPair.walletToIdentityKey(wallet, 0, 0)
    //
    ...
    //   const key = await sdk.keyPair.mnemonicToSeed(mnemonic)
    //
    //   expect(key.privateKey).toEqual(Uint8Array.from([89, 255, 64, 41, 202, 170, 83, 68, 135, 58, 161, 107, 130, 20, 3, 50, 69, 16, 108, 104, 32, 68, 13, 100, 225, 24, 79, 20, 193, 184, 238, 55]))
    // })
  })
